...
class SACagent(object):
    ...
    def pretreat(self, state1, state2, state3):
        ...
        self.pose_h1 = Dense(30, activation='relu')
        self.pose_h2 = Dense(40, activation='relu')

        #pos
        self.flatten3 = Flatten() # 1,12 or 4,1,12
        self.state_h1 = Dense(30, activation='relu')
        self.state_h2 = Dense(40, activation='relu')

        depth_features = self.conv1(state1)
        ...
        pos_features = self.flatten2(state2)
        pos_features = self.pose_h1(pos_features)
        pos_features = self.pose_h2(pos_features)
    ...
    def train(self, max_episode_num):
        ...
        for ep in range(int(max_episode_num)):
            ...
            while not done:
                ...
                # 이 부분에서 state 에 대해서 전처리를 진행하여야 한다.  

                state = pretreat(state["depth"], state["position"], state["position_state"])
