...
class OrbitNavigator:
    ...
    def start(self):
        ...
        # self.client를 사용해 드론의 현 위치를 가져옴. getMultirotorState는 드론의 상태를 가져오며, kinematics_estimated 는 운동학적 정보를 지닌다.
        start = self.client.getMultirotorState().kinematics_estimated.position
        
        # 드론의 착륙 상태를 나타내는 값이다. ex) airsim.landedState.landed 라는 값을 반환할 수 있다.
        ...
        while count < self.iterations:
            ...
            now = time.time()
            speed = self.speed
            # diff = 현재 시간과 비행 시작 시간의 차이를 계한
            diff = now - self.start_time
            if diff < ramptime:
            # speed를 현재 경과 시간 / ramptime으로 선형적으로 속도 증가를 한다. (ramptime 은 고정적. diff가 벌어질 수록 속도는 빨라짐)
